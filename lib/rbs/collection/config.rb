...
module RBS
  module Collection
    ...
    class Config
      ...
      def self.generate_lockfile(config_path:, definition:, with_lockfile: true, only: [])
        config = from_path(config_path)
        lockfile = LockfileGenerator.generate(config: config, definition: definition, with_lockfile: with_lockfile, only: only)
